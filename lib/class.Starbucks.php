...
class Starbucks {
	...
	public function update() {
		...
		if (!$this->dollar_balance){
			$this->dollar_balance = $this->parse_data($htmlBalance, 'card_dollar_balance');
		}
	...
	public function checkBalance() {
		...
		curl_setopt_array($ch, array(
			...
			CURLOPT_COOKIEJAR => $this->cookies_location,
			CURLOPT_POST => true,
			CURLOPT_POSTFIELDS => 'balance=update',
			CURLOPT_URL => 'https://www.starbucks.com/account/updatebalance'
		));
		$http_result = curl_exec($ch);
		curl_close($ch);
		return $http_result;
